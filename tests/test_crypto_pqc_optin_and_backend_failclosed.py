...
def test_verify_payload_backend_branch_rejects_non_string_sig(monkeypatch) -> None:
    ...
    kp = QIDKeyPair(
        algorithm=c.ML_DSA_ALGO,
        secret_key=c._b64encode(b"s" * 64),
        public_key=c._b64encode(b"p" * 32),
    )
...
def test_verify_payload_backend_branch_hybrid_rejects_non_string_sigs(monkeypatch) -> None:
    ...
    kp = QIDKeyPair(
        algorithm=c.HYBRID_ALGO,
        secret_key=c._b64encode(b"s" * 64),
        public_key=c._b64encode(b"p" * 32),
    )

    sig_env = _env_sig(
        {"v": "1", "alg": c.HYBRID_ALGO, "sigs": {c.ML_DSA_ALGO: "AA==", c.FALCON_ALGO: 123}}
    )
