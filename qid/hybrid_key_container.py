...
def _b64_std_encode(b: bytes) -> str:
    # IMPORTANT: tests use base64.b64decode(), so we must output standard base64 WITH padding.
    return base64.b64encode(b).decode("ascii")


def _b64_std_decode(s: str) -> bytes:
...
def public_view_dict(obj: Union[HybridKeyContainer, Mapping[str, Any], str]) -> Dict[str, Any]:
    ...
    Accepts:
      - HybridKeyContainer
      - dict / mapping
      - encoded base64 container string
    """
    if isinstance(obj, str):
        c = decode_container(obj)
        return {
            "v": c.v,
            "alg": c.alg,
            "kid": c.kid,
    ...
    if isinstance(obj, HybridKeyContainer):
        return {
            ...
            "alg": obj.alg,
            "kid": obj.kid,
            "ml_dsa": {"alg": obj.ml_dsa.alg, "public_key": obj.ml_dsa.public_key},
            "falcon": {"alg": obj.falcon.alg, "public_key": obj.falcon.public_key},
        }

    d = cast(Mapping[str, Any], obj)
...
def _validate_secret_key_types(d: Mapping[str, Any]) -> None:
    ml = cast(Mapping[str, Any], d.get("ml_dsa") or {})
    fa = cast(Mapping[str, Any], d.get("falcon") or {})

    ml_sk = ml.get("secret_key", None)
    fa_sk = fa.get("secret_key", None)
...
def from_dict(d: Mapping[str, Any]) -> HybridKeyContainer:
    ...
    ml_sk = cast(Optional[str], ml.get("secret_key", None))
    fa_sk = cast(Optional[str], fa.get("secret_key", None))

    return HybridKeyContainer(
        v=int(d["v"]),
        alg=str(d["alg"]),
        kid=str(d["kid"]),
        ml_dsa=KeyComponent(alg=str(ml["alg"]), public_key=str(ml["public_key"]), secret_key=ml_sk),
        falcon=KeyComponent(alg=str(fa["alg"]), public_key=str(fa["public_key"]), secret_key=fa_sk),
...
def encode_container(container: Union[HybridKeyContainer, Mapping[str, Any]]) -> str:
    """
    Encode container dict/object as base64(canonical_json(container_dict)).
...
def build_container(*args: Any, **kwargs: Any) -> HybridKeyContainer:
    """
    Supports TWO call styles expected by tests:
    ...
    Also supports alternate kw names used elsewhere:
      alg=HYBRID_ALGO, ml_dsa_pub=..., falcon_pub=...
    """
    if args:
